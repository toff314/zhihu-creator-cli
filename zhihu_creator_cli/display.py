...
def show_recommended_columns(data: dict, json_mode: bool = False) -> None:
    """Display recommended columns list."""
    if json_mode:
        _json_out(data)
        return
    ...
    if not columns:
        console.print("[yellow]暂无专栏[/yellow]")
        return
    ...
    table.add_column("ID/Slug", style="dim", width=20)
...
def show_user_columns(data: dict, json_mode: bool = False) -> None:
    """Display user's own columns list."""
    if json_mode:
        _json_out(data)
        return
    ...
    table.add_column("ID/Slug", style="dim", width=22)
    table.add_column("名称", min_width=30)
    table.add_column("文章数", justify="right", width=8)
    table.add_column("关注者", justify="right", width=8)
...
def show_search_columns(data: dict, json_mode: bool = False) -> None:
    ...
    results = data.get("data", [])
    if not results:
        console.print("[yellow]未搜索到专栏[/yellow]")
        return
    ...
    for item in results:
        ...
        # Remove HTML tags from title (search results have <em> tags)
        title = obj.get("title", "-")
        title = re.sub(r"<[^>]+>", "", title)
